...
/* ---------- tiny inline icon set (1.6 stroke, currentColor) ---------- */
const Icon = ({ name, size = 16, style }) => {
  const p = { width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.7, strokeLinecap: "round", strokeLinejoin: "round", style };
  switch (name) {
...
/* ---------- status helpers ---------- */
const Dot = ({ status, lg }) => <span className={`dot ${status}${lg ? " lg" : ""}`} />;
const COLOR = { green: "var(--green-dot)", yellow: "var(--yellow-dot)", red: "var(--red-dot)" };
const COLOR_TX = { green: "var(--green)", yellow: "var(--yellow)", red: "var(--red)" };
const COLOR_SOFT = { green: "var(--green-soft)", yellow: "var(--yellow-soft)", red: "var(--red-soft)" };

/* ---------- sparkline ---------- */
const Sparkline = ({ data, color = "var(--ink)", w = 96, h = 30, fill = false }) => {
...
/* ---------- progress bar (linear) ---------- */
const Bar = ({ value, status = "green", height = 6, showTrack = true, target }) => (
  <div style={{ position: "relative", height, borderRadius: 999, background: showTrack ? "var(--hairline-2)" : "transparent", overflow: "hidden" }}>
    <div style={{ position: "absolute", inset: 0, width: `${Math.min(value, 1) * 100}%`, background: COLOR[status], borderRadius: 999, transition: "width .8s cubic-bezier(.2,.7,.3,1)" }} />
  </div>
);

/* ---------- delta pill ---------- */
const Delta = ({ dir, children }) => {
  const up = dir === "up";
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11.5, fontWeight: 600,
      color: up ? "var(--green)" : "var(--red)" }} className="num">
      <Icon name={up ? "arrow-up" : "arrow-down"} size={12} />{children}
    </span>
  );
};

/* expose */
Object.assign(window, { Icon, Dot, Sparkline, Bar, Delta, COLOR, COLOR_TX, COLOR_SOFT, React });
