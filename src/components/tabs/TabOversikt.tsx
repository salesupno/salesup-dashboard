...
// ---- Invoice categories: which business line did the revenue come from? ----
function InvoiceCategories({ data }: { data: InvoiceSummary }) {
  ...
  const present = SALES_CATEGORY_ORDER
    ...
    .filter((c): c is InvoiceCategory => !!c)

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
        <span style={{ fontSize: 13.5, color: "var(--ink-3)", fontWeight: 700 }}>
          Fakturaer sendt · {data.total.count} stk
        </span>
        <span className="num" style={{ fontSize: 13.5, fontWeight: 800, color: "var(--ink-2)" }}>
          {oFmt(data.total.amount)}
        </span>
      </div>

      {present.length === 0 && (
        <div style={{ fontSize: 13, color: "var(--ink-3)", fontWeight: 600 }}>
          Ingen fakturaer i perioden.
        </div>
      )}
      ...
      {present.map((c) => (
        <div key={c.id} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
            ...
            <span className="num" style={{ fontSize: 13.5, fontWeight: 800 }}>{oFmt(c.amount)}</span>
          </div>
          ...
          <div style={{ fontSize: 12, color: "var(--ink-3)", fontWeight: 600 }}>
            ...
            {c.top.length > 0 && <> · {c.top.slice(0, 3).map((t) => t.name).join(", ")}{c.top.length > 3 ? " m.fl." : ""}</>}
          </div>
        </div>
      ))}
    </div>
        ...
        <div className="card" style={{ padding: "22px 30px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, alignItems: "center" }}>
            ...
            <div>
              ...
              <SalesFunnel meetings={periodMeetings} closed={periodWins} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ background: "var(--ink)", color: "#fff", borderRadius: 18, padding: "16px 20px" }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 18, color: "rgba(255,255,255,.65)", fontWeight: 700 }}>closed</span>
                </div>
              </div>
          ...
          <div style={{ borderTop: "1px solid var(--hairline)", paddingTop: 14 }}>
            <InvoiceCategories data={invoices} />
          </div>
        </div>
